...
const search = `      // High-Frequency Market Physics & Organic Wicks (Runs every frame at ~60 FPS)
...
const replace = `      if (rawLastCandleRef.current && targetPriceRef.current > 0) {
          if (currentInterpolatedPriceRef.current === 0) {
              currentInterpolatedPriceRef.current = targetPriceRef.current;
          } else {
...
if (code.includes(search)) {
    ...
    console.log("Reverted frontend chaos!");
} else {
