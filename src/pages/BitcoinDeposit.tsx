...
export default function BitcoinDeposit() {
  ...
  const amountCrypto = searchParams.get('amount') || '0.0015';
  const currency = searchParams.get('currency') || 'Bitcoin (BTC)';
  ...
  const btcWalletAddress = appConfig.btcAddress || "PLEASE_CONTACT_ADMIN_FOR_ADDRESS";

  useEffect(() => {
    if (currentUser && btcWalletAddress && !hasAutoSubmitted.current) {
      ...
      const autoSubmit = async () => {
        try {
          ...
          console.log("Auto-submitted pending Bitcoin deposit request:", dDoc.id);
        } catch (err) {
          console.error("Auto Bitcoin deposit failed:", err);
        }
      };
      autoSubmit();
    }
  }, [currentUser, btcWalletAddress, amountCrypto]);
  ...
  const walletAddress = appConfig.btcAddress || "PLEASE_CONTACT_ADMIN_FOR_ADDRESS";
  const qrCodeUrl = appConfig.btcQrCode || "https://i.postimg.cc/ZKN9zFGL/IMG-20260804-151047.png";
  ...
  return (
    <div className="min-h-screen bg-[#1a1f2e] text-[#E1E6F0] font-sans flex flex-col items-center">
      <SEO title="Deposit Bitcoin (BTC)" description="Secure Bitcoin payment gateway." />
      ...
      <div className="w-full max-w-[480px] min-h-screen flex flex-col pb-10">
        ...
        <header className="flex items-center justify-between px-4 py-6 border-b border-white/5">
          ...
          <h1 className="text-white font-black text-xl tracking-tight">
            Deposit {amountCrypto} BTC via {currency}
          </h1>
          <button onClick={() => navigate(-1)} className="text-gray-400 hover:text-white transition-colors">
            <Icons.X size={28} />
          </button>
            ...
            {isSuccess ? (
              ...
              >
                 ...
                 <p className="text-gray-400 text-sm leading-relaxed mb-8">
                   Your Bitcoin (BTC) deposit request has been submitted. It will be reviewed by our admin and credited to your balance shortly.
                 </p>
                 
                 <div className="w-full bg-white/5 rounded-2xl p-5 flex flex-col gap-3 mb-8">
                    <div className="flex justify-between text-[11px] font-black uppercase tracking-wider">
                       <span className="text-gray-400">Total Amount</span>
                       <span className="text-[#F7931A]">{amountCrypto} BTC</span>
                    </div>
                 </div>
              ...
              >
                <div className="px-4 flex flex-col gap-2 mt-2">
                  <div className="bg-[#fbbf24]/10 border border-[#fbbf24]/20 p-4 rounded-md flex gap-4 items-start">
                     ...
                     <p className="text-[13px] font-bold text-gray-300 leading-tight">
                       Only Bitcoin (BTC). Do not send other coins — they will be lost.
                     </p>
                  </div>
                ...
                <div className="mx-4 mt-6 bg-[#232936] rounded-2xl p-6 border border-white/5 flex flex-col items-center">
                   <div className="flex w-full gap-6 items-center">
                      ...
                      <div className="flex flex-col gap-1.5 flex-1">
                         <p className="text-[13px] font-bold text-gray-400">
                            To complete the payment transfer <span className="text-white font-black">{amountCrypto} BTC</span> to address
                         </p>
                   ...
                   <div className="grid grid-cols-2 gap-3 w-full mt-8">
                      <button 
                        onClick={() => copyToClipboard(amountCrypto, 'Amount')}
