...
export default function LtcDeposit() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const amountCrypto = searchParams.get('amount') || '0.05';
  ...
  const ltcWalletAddress = appConfig.ltcAddress || "LQ41bM2B892pfDX1suYe15hmsDuozgyZfU";

  useEffect(() => {
    if (currentUser && ltcWalletAddress && !hasAutoSubmitted.current) {
      ...
      const autoSubmit = async () => {
        try {
          ...
          console.log("Auto-submitted pending LTC deposit request:", dDoc.id);
        } catch (err) {
          console.error("Auto LTC deposit failed:", err);
        }
      };
      autoSubmit();
    }
  }, [currentUser, ltcWalletAddress, amountCrypto]);
  ...
  const walletAddress = appConfig.ltcAddress || "LQ41bM2B892pfDX1suYe15hmsDuozgyZfU";
  const qrCodeUrl = appConfig.ltcQrCode || "https://i.postimg.cc/9FCX4MCs/IMG-20260805-125156.png";
  ...
  return (
    <div className="min-h-screen bg-[#1a1f2e] text-[#E1E6F0] font-sans flex flex-col items-center">
      <SEO title="Deposit Litecoin (LTC)" description="Secure Litecoin payment gateway." />
            ...
            {isSuccess ? (
              ...
              >
                 ...
                 <p className="text-sm text-gray-400 mb-6 leading-relaxed">
                   Your Litecoin (LTC) deposit of <strong className="text-white">{amountCrypto} LTC</strong> has been submitted. Our automated system or admin will verify and credit your balance within a few minutes.
                 </p>
                 <button 
                   onClick={() => navigate(-1)}
              ...
              >
                <div className="bg-[#232936] rounded-3xl p-6 border border-white/5 shadow-2xl relative overflow-hidden">
                   <div className="absolute top-0 right-0 bg-[#345D9D]/10 text-[#345D9D] font-extrabold text-[10px] uppercase tracking-wider px-3 py-1.5 rounded-bl-2xl border-l border-b border-[#345D9D]/20">
                     Litecoin Network
                   </div>

                   <div className="flex items-center gap-3 mb-6">
                     <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center p-2">
                       <img src="https://i.postimg.cc/ZY6XyxqZ/images-(2).png" alt="LTC" className="w-full h-full object-cover rounded-xl" />
                     </div>
                     <div>
                       <h3 className="font-black text-lg text-white">Litecoin Deposit</h3>
                       <p className="text-xs text-gray-400">Min deposit: <span className="text-[#FFE24C] font-bold">0.05 LTC</span></p>
                     </div>
                   </div>
                   ...
                   <div className="bg-[#1a1f2e] border border-white/5 rounded-2xl p-4 flex flex-col items-center mb-6">
                     <div className="w-44 h-44 bg-white p-3 rounded-xl shadow-inner mb-4 flex items-center justify-center">
                       <img src={qrCodeUrl} alt="LTC QR" className="w-full h-full object-contain" />
                     </div>
                     <span className="text-xs text-gray-400 mb-1">Scan QR code to pay</span>
                     <div className="flex items-center gap-2 text-xs font-mono text-[#345D9D] bg-[#345D9D]/10 px-3 py-1 rounded-full">
                       <span>Time remaining: {formatTime(timeLeft)}</span>
                     </div>
                   </div>

                   <div className="space-y-2 mb-6">
                     <label className="text-[11px] font-black uppercase text-gray-400 tracking-wider">Litecoin Wallet Address</label>
                   ...
                   <div className="space-y-2 mb-6">
                     ...
                     <input 
                       ...
                       className="w-full bg-[#1a1f2e] border border-white/10 rounded-xl p-3 text-xs text-white font-mono outline-none focus:border-[#345D9D]"
                     />
                   </div>
                ...
                <div className="bg-[#232936] rounded-2xl p-5 border border-white/5 text-xs text-gray-400 space-y-2">
                   ...
                   <ul className="list-disc pl-4 space-y-1">
                     <li>Send only Litecoin (LTC) to the address above.</li>
                     <li>Minimum deposit amount is <strong className="text-white">0.05 LTC</strong>.</li>
